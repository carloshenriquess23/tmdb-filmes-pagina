...
export default function FilmeInfsPage(){
    ...
    const [fundoFilme,setFundoFilme]=useState("");
    ...
    async function respInfsFilme(){
        ...
        setInfsFilme(resp);
        setPoster(formatarImagemFilme(resp.poster_path));
        setFundoFilme(formatarImagemFilme(resp.belongs_to_collection.backdrop_path));
    }
    ...
    return(
        ...
        <div className='informacoes-filme-page'>
            ...
            <section className='infs-filme'>
                ...
                <div className='container-backdrop' style={{backgroundImage: `url(${fundoFilme})`}}></div>
                
                <img src={poster} alt="Filme Poster" id='poster'/>

                <div className='container-infs'>

                    <div className='subcontainer-titulo'>
                        <h3 className='titulo'>{infsFilme.title}</h3>
                        <h4 className='titulo2'>{infsFilme.tagline}</h4>
                    </div>

                    <div className='subcontainer-infs'>

                        <p className='destacar'> Lançado em {infsFilme.release_date} </p>
                        <p id='avaliacao'> <span className='destacar'>Avaliação:</span> {infsFilme.vote_average} 
                        ...
                        <p> <span className='destacar'>Sinopse:</span> {infsFilme.overview}</p>
                    </div>
                </div>
            </section>
            ...
            <section className='trailers-filme'>
                ...
                <div className='listagem-videos'>
                    {trailersFilme.map(item => 
                        <iframe src={"https://www.youtube.com/embed/"+item.key} width="500" height="300">
